...
Getting our final *LU* decomposition is now fairly simple. The /elimination matrix/ we got from Gaussian elimination is equal to *L^{-1}*. So we just need a quick helper function to invert *L* matrices by flipping the sign on all the factors.

#+BEGIN_SRC emacs-lisp :results output silent :session :tangle matrix.el
  (defun matrix-L-inverse (matrix)
...
And a little wrapper function will give us the ( *P L U* )

#+BEGIN_SRC emacs-lisp :results output silent :session :tangle matrix.el
  (defun matrix-LU-decomposition (matrix)
    "Get the LU decomposition of a matrix. Returns a list of ( P L U )"
    (let
        (( gaussian-elimination-matrices (matrix-gaussian-elimination-with-partial-pivoting matrix)))
      (list
       (second gaussian-elimination-matrices)
       (matrix-L-inverse (first gaussian-elimination-matrices))
       (third gaussian-elimination-matrices))))
#+END_SRC
