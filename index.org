...
*Note2*: Again, everything is transposed for convenience.. which is not very ergonomic
#+END_QUOTE
...
#+BEGIN_SRC emacs-lisp :results output silent :session :tangle matrix.el
  (defun matrix-gram-schmidt (A-transpose)
    ...
    (cond ((= 1 (matrix-rows A-transpose)) ;; base cas
          ...
          (t ;;recursive case
           (let* ((basis (matrix-gram-schmidt
                  ...
                  (orthogonal-component (matrix-get-orthogonal-component
                                         basis
                                         (matrix-transpose 
                                          (matrix-get-row A-transpose (1- (matrix-rows A-transpose)))))))
...
#+END_SRC
 ...
 So how do we express *A* in the term of the new orthogonal basis *Q*? We just look at the equations we've used and work backwards. We start again with breaking up the linearaly independent columns of *A* into the orthogonal components and the component in the span of the previous *k* columns
...
But when building *R* we already have /q_{k+1}/ and we are finding a way to get our column of *A* - /x_{k+1,orthogonal}/. So combining the two we get
...
#+BEGIN_SRC emacs-lisp :results output silent :session :tangle matrix.el
  (defun matrix-QR-decomposition (matrix)
    "template"
    (cond 
   ))
