...
So our method will still go reducing the matrix column by column and building up *U*, but in parallel we will be building *L* and *P*. So the result will be the three matrices (*L* *P* *U*).
#+BEGIN_SRC emacs-lisp :results output silent :session :tangle matrix.el
  (defun matrix-LU-decomposition (matrix)
    "Perform Gaussian elimination with partial pivoting on MATRIX and return the list (L P U), representing the LU-decomposition "
    (let
        ((rank
          (matrix-rows matrix)))
      (defun matrix-LU-decomposition-rec (L-matrix
        ...
        (cond
         ((equal
           column-to-reduce
           rank)
          (list L-matrix P-matrix reduced-matrix))
         (t
          (let
...
#+BEGIN_SRC emacs-lisp :results output silent :session :tangle matrix.el
  (defun matrix-solve-input (matrix output-vector)
    ...
    (let* ((LU-decomposition (matrix-LU-decomposition matrix))
           (permuted-output-vector (matrix-product (second LU-decomposition) output-vector))
           (intermediate-y-vector (matrix-forward-substitution (first LU-decomposition) permuted-output-vector)))
