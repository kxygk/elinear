...
#+BEGIN_SRC emacs-lisp :results output silent :session :tangle matrix.el
  (defun matrix-partial-pivot (matrix pivot-column)
      "Adjust the pivot in the PIVOT-COLUMN to be the maximal value in that column"
...
If *G_{n}* are the /elementary lower-triangular matrices/ that clear the columns and *F_{n}* are the *Type I* pivot adjustments, then the steps compose into:  *G_{1}F_{1}G_{2}F_{2}..G_{r}F_{r}A* where you can pair up each elimination and pivot steps as they complete each column *(G_{1}F_{1})_{col_1}(G_{2}F_{2})_{col_2}..(G_{r}F_{r})_{col_r}A*
