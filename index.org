...
** Chaining problems through matrix composition and taxing the farmer
...
** Unit Column
The next step is building the *unit column* (denoted *e_{/j/}*) - which contains a *1* in the given postion (here: /j/) and *0s* everwhere else. Its transpose is naturally the *unit row*\\
