...
To build an upper triangular matrix we need to zero out values below the diagonal. The way we're going to do is by reflecting those columns onto coordinate axis. One a vector is on a coordinate axis then it's other components are naturally zero!
...
#+BEGIN_SRC emacs-lisp :results output silent :session :tangle matrix.el
  ...
  (defun matrix-elementary-coordinate-reflector (column-vector coordinate-axis)
    ...
    (matrix-elementary-reflector (matrix-add column-vector
