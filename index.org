...
This was tested in a little demo program written in Clojure. Fitting a high order polynomial over some random points the *A^{T}Ax=A^{T}b* solution (light blue) gives a broken solution much quicker than the stable solution (dark blue)
