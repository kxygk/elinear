...
~pages 185-186~ introduces the Vandermonde matix which provides us with a way to solve for polynomials that fit a set of given points. Polynomials are equations of the form:
\begin{equation}
y=a_{1}+a_{2}x+a_{3}x^{2}+a_{4}x^{3}+...
\end{equation}
So given some ~(x,y)~ points we can try to solve for these *a_{n}* factors. And we can choose the number of terms in the polynomial to insure we have a nonsingular matrix that's solvable (see bottom of ~page 185~)

The Vandermonde matrix is built from a list of ~x~'s and their exponents
#+BEGIN_SRC emacs-lisp :results output silent :session :tangle matrix.el
...
#+END_QUOTE
* Least Squares
Now we have a mechanism to solve square linear systems we need to extend *Ax=b* to the overdefined case where we have more linear equations than parameters (ie. a skinny matrix). If each equation is thought of as representing a "measurement" then in other words when we have more measurements than function paramters to solve for. In the general case (with the addition of noise) no explicit solution will exist for *A_{skinny}x=b* so we need to find the next-best-thing. Here there is a bit confusion in terminology b/c we think of our measurements as being an "input" and "output", but here the thing we are solving for (ie. what I've been calling an input) are the parameters of the function. So given a function *f(x)* we have measurements that say we passed in certain values *a* and got out values *b* and then we want to solve for the function parameters *p*. In matrix form (as for the Vandermonde case) we would write that out as *A_{f(a)}p=b*. But since there are too many measurements no exact solution will exist where *f(a_{i})=b_{i}* will always hold true. Instead we will get close-by values *f(a)=b_{estimate}*. Our goal is to solve for parameters *p* that will get us as close as possible. Ie *f(a)-b=b_{estimate}-b* or *A_{f(a)}p-b* should be as small as possible. To make the situation easier numerically we take the square of the difference (so all values are positive) *(f(a)-b)^{2}=(b_{estimate}-b)^{2}* or *(A_{f(a)}p-b)^{2}* . Minimizing a function is done by take the derivative and setting it equal to zero and then solving for the parameters. What /page 226/ shows is that the derivative of *(A_{f(a)}p-b)^{2}*  will give us the matrix form *A^{T}Ax=A^{T}b* which is smaller, square and has a solution. This amazingly holds for all linear equations.

** Numerically stable solution
Unfortunately this solution involves multiplcation so it has some numerical issues. A better solution is presented in *Exercise 4.6.9* with the equations
...
This was tested in a [[../asparapiss/][little demo program]] written in Clojure. Fitting a high order polynomial over some random points the *A^{T}Ax=A^{T}b* solution (light blue) gives a broken solution much quicker than the stable solution (dark blue)
